...
//        System.out.println("id:"+this.id+" balance:"+this.balance +" affordableOptionsSize:"+this.affordableOptionsAsNodes.size());
    ...
    {
        List<Node> result = new ArrayList<>(currentNode.ANL.size()+1);
        result.add(currentNode);
        result.addAll(currentNode.ANL);
    ...
    static void updateStatics(List<AIProto.Agent> everyAgent, AIProto.Team team , Agent self,boolean firstMove)
    {
        ...
        if (tempAllyCops.size()!=0 && (self.type==AIProto.AgentType.POLICE)){

            for (int i=0;i!=tempAllyCops.size();i++){
                if (tempAllyCops.get(i).id==self.id){
                    shouldAddSelf=false;
                }
            }
            if (shouldAddSelf){tempAllyCops.add(new Agent(self));}

            allyCops = tempAllyCops;

//            if (allyCops.size()==0 || firstMove){
//                allyCops = tempAllyCops;
//                for (int i=0;i!=allyCops.size();i++){
//                    allyCops.get(i).balance = initialBalance;
//                    allyCops.get(i).update();
//                }
//            }
//            else{
//                double price;
//                for (int i=0;i!=allyCops.size();i++)
//                {
//                    for (int c=0;c!=tempAllyCops.size();c++){
//                        if (tempAllyCops.get(c).id == allyCops.get(i).id){
//                            if (tempAllyCops.get(c).currentNode == allyCops.get(i).currentNode){
//                                price = 0;
//                            }
//                            else{
//                                price = Edge.findEdgeByNodes(tempAllyCops.get(c).currentNode,allyCops.get(i).currentNode).price;
//
//                            }
//                            allyCops.get(i).currentNode = tempAllyCops.get(c).currentNode;
//                            allyCops.get(i).balance += (-price + (copsIncome*2));
//                            allyCops.get(i).update();
//                            break;
//                        }
//                    }
