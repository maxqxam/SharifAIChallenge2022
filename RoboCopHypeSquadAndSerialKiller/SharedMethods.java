...
//            System.out.println("Added the shortestPath from node "+fromNode.id+" to node "+toNode.id+" , to the solution collection!");
        ...
        static int getIndexIfExists(Node fromNode,Node toNode)
        {
            ...
            {
                if ((fromNode==stepListList.get(i).fromNode) && (toNode==stepListList.get(i).toNode)) {return i;}
            }
            return -1;
        }
    ...
    final private static  int maxPredictionSize=5000;

    private static void MAGIC(List<Integer> allNumbers, int vecIndex, List<Integer> listSoFar,List<List<Integer>> resultList) {
        if (resultList.size()>=maxPredictionSize){return;}

        if (vecIndex >= allNumbers.size()) {
            resultList.add(listSoFar);
            return;
        }

        for (int i = 0; i != allNumbers.get(vecIndex); i++) {
            List<Integer> temp = new ArrayList<>(listSoFar);
            temp.add(i);
            MAGIC(allNumbers, vecIndex + 1, temp,resultList);
        }

    }


    static List<List<Integer>> getAllPosibilities(List<Integer> indexList){
        List<List<Integer>> resultList = new ArrayList<>();

        MAGIC(indexList,0,new ArrayList<>(),resultList);
